...
export async function handleConnectCommand(ctx: MyContext): Promise<void> {
  ...
  if (!chatId) {
    console.error("handleConnectCommand, chatId is null");
    return;
  }
  ...
  const botMessage = await ctx.reply("connect", {
    reply_markup: {
...
export async function handleSendTXCommand(ctx: MyContext): Promise<void> {
  const chatId = ctx.msg?.chat.id;
  if (!chatId) {
    console.error("handleSendTXCommand, chatId is null");
    return;
  }
  const connector = getConnector(chatId);
...
export async function handleDisconnectCommand(ctx: MyContext): Promise<void> {
  const chatId = ctx.msg?.chat.id;
  if (!chatId) {
    console.error("handleDisconnectCommand, chatId is null");
    return;
  }
...
export async function handleShowMyWalletCommand(ctx: MyContext): Promise<void> {
  const chatId = ctx.msg?.chat.id;
  if (!chatId) {
    console.error("handleShowMyWalletCommand, chatId is null");
    return;
  }
